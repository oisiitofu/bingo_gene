...
別ユーザー・別端末として複数タブを検証する場合は、タブごとに次のように指定します。

- `?onlineMock=1&onlineMockUser=master&onlineMockDevice=device-a`
- `?onlineMock=1&onlineMockUser=guest&onlineMockDevice=device-b`
...
オンライン戦績の差分計算とバックアップ形式は、次のコマンドでローカル検証できます。

```powershell
npm test
```

本番Firebaseの作成・参加・権限拒否・管理削除・GHOST削除を確認する場合は、明示フラグを付けてスモークテストを実行します。テスト専用の匿名ユーザーと`SMOKE-...`部屋を作成し、終了時に削除します。

```powershell
$env:TEAM_BINGO_FIREBASE_SMOKE="1"
