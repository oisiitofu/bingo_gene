(function bootstrapTerritoryMap3D(global) {
  ...
  const LANDMARK_ASSETS = Object.freeze({
    fortress: "images/territory/realistic/fortress.png",
    ...
    waterRuins: "images/territory/realistic/water-ruins.png",
    village: "images/territory/realistic/village.png",
    stormSpire: "images/territory/realistic/storm-spire.png",
    darkCitadel: "images/territory/realistic/dark-citadel.png"
  });
  ...
  function create(container, options = {}) {
    ...
    function createSharedAssets() {
      ...
      const materials = {
        ...
        stone: new THREE.MeshStandardMaterial({ color: 0x77736b, roughness: .84, metalness: .08 }),
        roof: new THREE.MeshStandardMaterial({ color: 0x333743, roughness: .72, metalness: .18 }),
      ...
      return { geometries, materials, terrainTextures, landmarkMaterials };
    }
    ...
    function createTile(tile) {
      ...
      const rimMaterial = new THREE.MeshBasicMaterial({
        ...
        transparent: true,
        opacity: tile.ownerId ? .4 : .08,
        side: THREE.DoubleSide,
        depthWrite: false
      });
      dynamicMaterials.push(rimMaterial);
      const rim = new THREE.Mesh(shared.geometries.rim, rimMaterial);
      rim.position.set(position.x, height + .012, position.z);
      rim.rotation.x = -Math.PI / 2;
      rim.rotation.z = Math.PI / 6;
      rim.userData.tileRim = true;
      tileRoot.add(rim);
      ...
      if (tile.eventId) addEventBeacon(detail, tile);
      tileRoot.add(detail);
    }
    ...
    function addTerrainObjects(group, tile) {
      ...
      if (tile.terrain === "wind") {
        ...
        if (seededValue(seed, "earth-landmark") > .48) {
          addLandmarkSprite(group, "village", 1.62, 1.15, .02, seed);
        } else {
        ...
        addLandmarkSprite(group, "waterRuins", 1.48, 1.08, .01, seed);
        ...
        addLandmarkSprite(group, "stormSpire", 1.28, 1.48, .01, seed);
      } else if (tile.terrain === "light") {
        if (seededValue(seed, "light-landmark") > .46) {
          addLandmarkSprite(group, "village", 1.58, 1.12, .01, seed);
        } else {
          addLandmarkSprite(group, "waterRuins", 1.42, 1.04, .01, seed);
        }
      } else if (tile.terrain === "dark") {
        addLandmarkSprite(group, "darkCitadel", 1.5, 1.2, .01, seed);
      }
    }
    ...
    function addFortress(group, tile, ownerColor) {
      const scale = tile.kind === "throne" ? 1.08 : (tile.kind === "base" ? .92 : .72);
      addLandmarkSprite(
        group,
        "fortress",
        tile.kind === "throne" ? 2.45 : (tile.kind === "base" ? 2.15 : 1.72),
        tile.kind === "throne" ? 1.84 : (tile.kind === "base" ? 1.62 : 1.3),
        .02,
        tile.id
      );
      addBanner(group, ownerColor, 1.16 * scale);
    }

    function addEventBeacon(group, tile) {
      const eventColor = EVENT_COLORS[tile.eventId] || 0xffffff;
      ...
      dynamicMaterials.push(material);
      const orb = new THREE.Mesh(shared.geometries.orb, material);
      orb.position.set(-.62, .3, .52);
      group.add(orb);
      animatedObjects.push({ object: orb, type: "beacon", phase: seededValue(tile.id, "event") * 6 });
    }
    ...
    function updateSelection() {
      ...
      tileMeshes.forEach((mesh, tileId) => {
        ...
        mesh.scale.y = selected ? 1.22 : 1;
        mesh.material.emissive.setHex(selected ? 0x403014 : 0x000000);
        mesh.material.emissiveIntensity = selected ? .55 : 0;
      });
    }
    ...
    function animate(time) {
      ...
      animatedObjects.forEach((entry) => {
        ...
        } else if (entry.type === "beacon") {
          entry.object.position.y = .31 + Math.sin(seconds * 2.3 + entry.phase) * .07;
          entry.object.material.emissiveIntensity = 1.05 + Math.sin(seconds * 3 + entry.phase) * .38;
        }
      });
      cameraPosition();
      renderer.render(scene, camera);
