  ...
  function resolveTarget(state, targetId, actions, playerStats, at, random) {
    ...
    if (!previousOwnerId && actions.length === 1) {
      ...
      pushLog(state, {
        ...
        sourceTileId: action.fromId,
        sourceTileIds: [action.fromId],
        tileId: targetId,
        eventId: event.id,
    ...
    pushLog(state, {
      ...
      sourceTileId: (sides.find((side) => side.sourceTileId !== targetId) || sides[0])?.sourceTileId || targetId,
      sourceTileIds: [...new Set(sides.map((side) => side.sourceTileId).filter((sourceTileId) => sourceTileId && sourceTileId !== targetId))],
      tileId: targetId,
      battleId: battle.id,
      eventId: event.id,
      hypeChanges,
