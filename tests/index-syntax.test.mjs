...
test("online lobby boot bypasses stale browser modules and retries once", () => {
  ...
  assert.match(html, /online-room\.js\?v=20260905-life-menu-142/);
  assert.match(html, /retry=\$\{Date\.now\(\)\}/);
  assert.match(serviceWorker, /20260905-life-menu-142/);
});
...
test("BINGO CITY is connected to the shared client, worker tick, rules, and offline shell", () => {
  ...
  assert.match(citySystem, /const FEATURE_REVISION = 11/);
  ...
  assert.match(citySystem, /function recordCityAlbum/);
  assert.match(cityMode, /CITY ARCHIVE/);
  ...
  assert.match(serviceWorker, /\.\/city-system\.js\?v=20260904-city-album-155/);
