...
test("BINGO CITY is connected to the shared client, worker tick, rules, and offline shell", () => {
  ...
  assert.match(cityMap, /function addDistrictIdentity/);
  assert.match(cityMap, /Math\.floor\(variant \/ 10\) % 10/);
  assert.match(citySystem, /function expandCatalog/);
  assert.match(citySystem, /visualSignature: `\$\{model\}:\$\{visualForm\}:\$\{visualTheme\}:\$\{visualEdition\}`/);
  assert.match(cityMode, /const BUILD_PAGE_SIZE = 12/);
  ...
  assert.match(html, /src="city-system\.js\?v=20260904-city-weather-154"/);
  assert.match(html, /src="city-map-3d\.js\?v=20260904-city-weather-154"/);
  assert.match(html, /src="city-mode\.js\?v=20260904-city-weather-154"/);
  ...
  assert.match(cityMode, /canEditActiveCity/);
  assert.match(cityMode, /この都市は閲覧専用です/);
  assert.match(online, /subscribeCity\(\)/);
  assert.match(online, /snapshot\.terrainRevision/);
  ...
  assert.match(citySystem, /const FEATURE_REVISION = 10/);
  ...
  assert.match(citySystem, /function cityStage/);
  assert.match(cityMap, /function createDistrictBoundaries/);
  assert.match(cityMap, /function addCityStageRoadDetail/);
  assert.match(cityMap, /function buildSignatureLandmark/);
  assert.match(cityMode, /city-district-chip/);
  assert.match(cityMode, /city-landmark-status/);
  ...
  assert.match(citySystem, /const CITY_POLICIES/);
  assert.match(cityMode, /MAYOR POLICY/);
  assert.match(citySystem, /const CITY_INTERACTIONS/);
  assert.match(cityMode, /CITY LINK/);
  assert.match(citySystem, /function cityEnvironment/);
  assert.match(cityMap, /function createWeatherEffect/);
  assert.match(citySystem, /const CITIZEN_CASTS/);
  assert.match(citySystem, /function cityPulse/);
  assert.match(cityMode, /市民・ニュース/);
  assert.match(citySystem, /function analyzeTraffic/);
  assert.match(citySystem, /metro-station/);
  assert.match(cityMap, /traffic\.congestion/);
  assert.match(citySystem, /const CITY_EVENTS/);
  assert.match(citySystem, /function processCityEvents/);
  assert.match(cityMode, /ACTIVE EVENT/);
  ...
  assert.match(serviceWorker, /\.\/city-system\.js\?v=20260904-city-weather-154/);
